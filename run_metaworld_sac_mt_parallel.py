...
def run_experiment(args, save_dir, exp_id = 0, seed = None):
    ...
    # --- ADD: separate evaluation env & core (same tasks as training) ---
    eval_mdp = SubprocVecEnv([
        make_env(env_name=env_name,
                 env_cls=env_cls,
                 train_tasks=benchmark.train_tasks,
                 horizon=horizon,
    ...
    eval_core = VecCore(agent, eval_mdp)
    ...
    if args.start_epoch == 0:
        ...
        # --- REPLACE: sequential random‐policy eval with vectorised eval_core ---
        eval_core.eval = True
        datasets, datasets_info = eval_core.evaluate(
        ...
        for c, key in enumerate(env_names):
            env_dataset = datasets[c]
            info = {k: v[c] for k, v in datasets_info.items()}
            min_J, max_J, mean_J, mean_disc_J, success_rate = get_stats(
                env_dataset, gamma, gamma_eval, dataset_info=info
            )
            ...
            metrics[key]["AverageDiscountedReturn"].append(mean_disc_J)
            ...
            single_logger.epoch_info(
                ...
                mean_J=mean_J, mean_discounted_J=mean_disc_J,
                success_rate=success_rate, log_alpha=log_alpha
            )
            if args.wandb:
                wandb.log({
                    ...
                    f'{key}/AverageDiscountedReturn': mean_disc_J,
        ...
        if args.wandb:
            wandb.log({"all_metaworld/SuccessRate": current_success_rate_avg / n_contexts},
                      step=0, commit=True)
    ...
    for n in trange(args.start_epoch, n_epochs):
        ...
        # --- REPLACE: sequential periodic eval with vectorised eval_core ---
        eval_core.eval = True
        datasets, datasets_info = eval_core.evaluate(
            n_episodes=n_episodes_test,
